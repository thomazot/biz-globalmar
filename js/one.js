...
$j(document)
    .ready(function ($) {
        ...
        $('.categories__all .a--0, .categories__all .box--1')
            .mouseenter(function () {
                $('body').addClass('categories--on')
            })
            .mouseleave(function () {
                $('body').removeClass('categories--on')
            })

        $('.categories .all-a--0').each(function () {
            var text = $.trim($(this).text())
            var ul = $(this).closest('li').find('.all-ul--1')

            if (ul.length) {
                ul.attr('data-name', text)
            }
        })
